...
//Criação da aresta 
bool criaAresta_nao(GRAFO* gr, int vi, int vf, int p){
    if(!gr) return(false);
...
int grau_no_entrada(GRAFO* gr, int no){
    ...
    criaAresta_nao(gr,1,3,4);
    criaAresta_nao(gr,1,4,2);
    criaAresta_nao(gr,1,5,3);
    criaAresta_nao(gr,2,3,4);
    criaAresta_nao(gr,2,4,-2);
    criaAresta_nao(gr,2,5,3);
     criaAresta_nao(gr,2,5,5);
      criaAresta_nao(gr,2,5,1);
